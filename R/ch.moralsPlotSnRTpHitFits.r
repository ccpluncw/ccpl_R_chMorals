...
#' @return .
#' @export
#' @examples ch.moralsPlotSnRTpHitFits(subOutData, "sn", "rtSlo", "rtInt", "rtR2", "phB", "phR2", xAll, filename)

ch.moralsPlotSnRTpHitFits <- function (snSummaryArray, snCol, rtSloCol, rtIntCol, rtR2Col, phBCol, phR2Col, xAll, filename = NULL) {
  ...
  par(lwd=2)
  hist(snSummaryArray[[rtR2Col]], main=NA, xlab= NA, ylab = NA,  xlim = c(-1,1), breaks=12)
  hist(snSummaryArray[[phR2Col]], main=NA, xlab= NA, ylab = NA,  xlim = c(0,1), breaks=12)
